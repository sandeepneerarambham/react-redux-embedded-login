...
class ChangeEmail extends Component {
  ...
  componentWillUpdate(nextProps, nextState) {
    ...
    if (error) {
      this.container.error(`Change Email failed: ${error}`, "Error occurred", {
  ...
  onSubmit(values) {
    const { email } = values;
    this.props.changeEmail({ newEmail: email }, () => {
...
function mapStateToProps(state) {
  const { error, timestamp, loading } = state.email;
  return {
    error,
    timestamp,
    loading
...
export default reduxForm({
  validate,
  form: "ChangeEmail"
})(connect(mapStateToProps, { ...actions } )(ChangeEmail));