...
class Home extends Component {
  ...
  componentWillUpdate(nextProps, nextState) {
    const { emailMsg, passwordMsg } = nextProps;
    if (emailMsg) {
      this.container.success(`${emailMsg}`, "Success", {
    ...
    return (
      ...
      <form className="form-horizontal col-xs-10 col-xs-offset-1" onSubmit={e => e.preventDefault()}>
        <div className="form-group">
          ...
          <Link to="/email" className="btn btn-primary btn-sm">Change Email</Link>
...
function mapStateToProps(state) {
  const { emailMsg, passwordMsg } = state.home;
  return {
    emailMsg,
    passwordMsg
  };
